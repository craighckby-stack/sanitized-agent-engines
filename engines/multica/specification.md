...
> **Source Origin**: [multica-ai/multica](https://github.com/multica-ai/multica)
...
## Engine 1: multica Lifecycle Kernel
...
## Engine 2: multica ReAct Loop Engine
...
export class multicaAgentLoopEngine {
  constructor(
    private ctx: multicaLifecycleContext,
...
## Engine 3: multica Unified Model Stream Adapter
...
export class multicaModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[multicaModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: multica Tool Sandbox & Virtual File System Engine
...
export class multicaVirtualFileSystem {
...
export class multicaToolSandbox {
  private vfs: multicaVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new multicaVirtualFileSystem(initialFiles);
  }
...
## Engine 5: multica Non-Linear Session Tree & Token Budget Engine
...
export class multicaSessionTreeEngine {
