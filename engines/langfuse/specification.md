...
> **Source Origin**: [langfuse/langfuse](https://github.com/langfuse/langfuse)
...
## Engine 1: langfuse Lifecycle Kernel
...
export class langfuseLifecycleContext {
  public readonly id: string;
  public readonly parent: langfuseLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: langfuseLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: langfuseLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[langfuseLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: langfuseLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[langfuseLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): langfuseLifecycleContext {
    return new langfuseLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[langfuseLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: langfuse ReAct Loop Engine
...
export class langfuseAgentLoopEngine {
  constructor(
    private ctx: langfuseLifecycleContext,
...
## Engine 3: langfuse Unified Model Stream Adapter
...
export class langfuseModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[langfuseModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: langfuse Tool Sandbox & Virtual File System Engine
...
export class langfuseVirtualFileSystem {
...
export class langfuseToolSandbox {
  private vfs: langfuseVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new langfuseVirtualFileSystem(initialFiles);
  }
...
## Engine 5: langfuse Non-Linear Session Tree & Token Budget Engine
...
export class langfuseSessionTreeEngine {
