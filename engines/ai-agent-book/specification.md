...
> **Source Origin**: [bojieli/ai-agent-book](https://github.com/bojieli/ai-agent-book)
...
## Engine 1: ai-agent-book Lifecycle Kernel
...
export class ai-agent-bookLifecycleContext {
  public readonly id: string;
  public readonly parent: ai-agent-bookLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: ai-agent-bookLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: ai-agent-bookLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[ai-agent-bookLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: ai-agent-bookLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[ai-agent-bookLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): ai-agent-bookLifecycleContext {
    return new ai-agent-bookLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[ai-agent-bookLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: ai-agent-book ReAct Loop Engine
...
export class ai-agent-bookAgentLoopEngine {
  constructor(
    private ctx: ai-agent-bookLifecycleContext,
...
## Engine 3: ai-agent-book Unified Model Stream Adapter
...
export class ai-agent-bookModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[ai-agent-bookModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: ai-agent-book Tool Sandbox & Virtual File System Engine
...
export class ai-agent-bookVirtualFileSystem {
...
export class ai-agent-bookToolSandbox {
  private vfs: ai-agent-bookVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new ai-agent-bookVirtualFileSystem(initialFiles);
  }
...
## Engine 5: ai-agent-book Non-Linear Session Tree & Token Budget Engine
...
export class ai-agent-bookSessionTreeEngine {
