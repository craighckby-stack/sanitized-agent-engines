...
> **Source Origin**: [ToolJet/ToolJet](https://github.com/ToolJet/ToolJet)
...
## Engine 1: ToolJet Lifecycle Kernel
...
export class ToolJetLifecycleContext {
  public readonly id: string;
  public readonly parent: ToolJetLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: ToolJetLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: ToolJetLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[ToolJetLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: ToolJetLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[ToolJetLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): ToolJetLifecycleContext {
    return new ToolJetLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[ToolJetLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: ToolJet ReAct Loop Engine
...
export class ToolJetAgentLoopEngine {
  constructor(
    private ctx: ToolJetLifecycleContext,
...
## Engine 3: ToolJet Unified Model Stream Adapter
...
export class ToolJetModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[ToolJetModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: ToolJet Tool Sandbox & Virtual File System Engine
...
export class ToolJetVirtualFileSystem {
...
export class ToolJetToolSandbox {
  private vfs: ToolJetVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new ToolJetVirtualFileSystem(initialFiles);
  }
...
## Engine 5: ToolJet Non-Linear Session Tree & Token Budget Engine
...
export class ToolJetSessionTreeEngine {
