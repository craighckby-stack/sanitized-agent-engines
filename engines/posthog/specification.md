...
> **Source Origin**: [PostHog/posthog](https://github.com/PostHog/posthog)
...
## Engine 1: posthog Lifecycle Kernel
...
export class posthogLifecycleContext {
  public readonly id: string;
  public readonly parent: posthogLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: posthogLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: posthogLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[posthogLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: posthogLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[posthogLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): posthogLifecycleContext {
    return new posthogLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[posthogLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: posthog ReAct Loop Engine
...
export class posthogAgentLoopEngine {
  constructor(
    private ctx: posthogLifecycleContext,
...
## Engine 3: posthog Unified Model Stream Adapter
...
export class posthogModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[posthogModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: posthog Tool Sandbox & Virtual File System Engine
...
export class posthogVirtualFileSystem {
...
export class posthogToolSandbox {
  private vfs: posthogVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new posthogVirtualFileSystem(initialFiles);
  }
...
## Engine 5: posthog Non-Linear Session Tree & Token Budget Engine
...
export class posthogSessionTreeEngine {
