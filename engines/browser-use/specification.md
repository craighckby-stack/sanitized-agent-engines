...
## Engine 1: browser-use Lifecycle Kernel
...
export class browser-useLifecycleContext {
  public readonly id: string;
  public readonly parent: browser-useLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: browser-useLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: browser-useLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[browser-useLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: browser-useLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[browser-useLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): browser-useLifecycleContext {
    return new browser-useLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[browser-useLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: browser-use ReAct Loop Engine
...
export class browser-useAgentLoopEngine {
  constructor(
    private ctx: browser-useLifecycleContext,
...
## Engine 3: browser-use Unified Model Stream Adapter
...
export class browser-useModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[browser-useModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: browser-use Tool Sandbox & Virtual File System Engine
...
export class browser-useVirtualFileSystem {
...
export class browser-useToolSandbox {
  private vfs: browser-useVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new browser-useVirtualFileSystem(initialFiles);
  }
...
## Engine 5: browser-use Non-Linear Session Tree & Token Budget Engine
...
export class browser-useSessionTreeEngine {
