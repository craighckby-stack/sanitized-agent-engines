...
> **Source Origin**: [mendableai/firecrawl](https://github.com/mendableai/firecrawl)
...
## Engine 1: firecrawl Lifecycle Kernel
...
export class firecrawlLifecycleContext {
  public readonly id: string;
  public readonly parent: firecrawlLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: firecrawlLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: firecrawlLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[firecrawlLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: firecrawlLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[firecrawlLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): firecrawlLifecycleContext {
    return new firecrawlLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[firecrawlLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: firecrawl ReAct Loop Engine
...
export class firecrawlAgentLoopEngine {
  constructor(
    private ctx: firecrawlLifecycleContext,
...
## Engine 3: firecrawl Unified Model Stream Adapter
...
export class firecrawlModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[firecrawlModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: firecrawl Tool Sandbox & Virtual File System Engine
...
export class firecrawlVirtualFileSystem {
...
export class firecrawlToolSandbox {
  private vfs: firecrawlVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new firecrawlVirtualFileSystem(initialFiles);
  }
...
## Engine 5: firecrawl Non-Linear Session Tree & Token Budget Engine
...
export class firecrawlSessionTreeEngine {
