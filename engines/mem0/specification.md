...
> **Source Origin**: [mem0ai/mem0](https://github.com/mem0ai/mem0)
...
## Engine 1: mem0 Lifecycle Kernel
...
export class mem0LifecycleContext {
  public readonly id: string;
  public readonly parent: mem0LifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: mem0LifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: mem0LifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[mem0LifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: mem0LifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[mem0LifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): mem0LifecycleContext {
    return new mem0LifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[mem0LifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: mem0 ReAct Loop Engine
...
export class mem0AgentLoopEngine {
  constructor(
    private ctx: mem0LifecycleContext,
...
## Engine 3: mem0 Unified Model Stream Adapter
...
export class mem0ModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[mem0ModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: mem0 Tool Sandbox & Virtual File System Engine
...
export class mem0VirtualFileSystem {
...
export class mem0ToolSandbox {
  private vfs: mem0VirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new mem0VirtualFileSystem(initialFiles);
  }
...
## Engine 5: mem0 Non-Linear Session Tree & Token Budget Engine
...
export class mem0SessionTreeEngine {
