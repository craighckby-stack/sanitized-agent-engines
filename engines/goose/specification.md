...
> **Source Origin**: [aaif-goose/goose](https://github.com/aaif-goose/goose)
...
## Engine 1: goose Lifecycle Kernel
...
export class gooseLifecycleContext {
  public readonly id: string;
  public readonly parent: gooseLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: gooseLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: gooseLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[gooseLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: gooseLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[gooseLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): gooseLifecycleContext {
    return new gooseLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[gooseLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: goose ReAct Loop Engine
...
export class gooseAgentLoopEngine {
  constructor(
    private ctx: gooseLifecycleContext,
...
## Engine 3: goose Unified Model Stream Adapter
...
export class gooseModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[gooseModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: goose Tool Sandbox & Virtual File System Engine
...
export class gooseVirtualFileSystem {
...
export class gooseToolSandbox {
  private vfs: gooseVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new gooseVirtualFileSystem(initialFiles);
  }
...
## Engine 5: goose Non-Linear Session Tree & Token Budget Engine
...
export class gooseSessionTreeEngine {
