...
> **Source Origin**: [xorbitsai/inference](https://github.com/xorbitsai/inference)
...
## Engine 1: inference Lifecycle Kernel
...
export class inferenceLifecycleContext {
  public readonly id: string;
  public readonly parent: inferenceLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: inferenceLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: inferenceLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[inferenceLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: inferenceLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[inferenceLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): inferenceLifecycleContext {
    return new inferenceLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[inferenceLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: inference ReAct Loop Engine
...
export class inferenceAgentLoopEngine {
  constructor(
    private ctx: inferenceLifecycleContext,
...
## Engine 3: inference Unified Model Stream Adapter
...
export class inferenceModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[inferenceModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: inference Tool Sandbox & Virtual File System Engine
...
export class inferenceVirtualFileSystem {
...
export class inferenceToolSandbox {
  private vfs: inferenceVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new inferenceVirtualFileSystem(initialFiles);
  }
...
## Engine 5: inference Non-Linear Session Tree & Token Budget Engine
...
export class inferenceSessionTreeEngine {
