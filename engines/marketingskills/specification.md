...
> **Source Origin**: [coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills)
...
## Engine 1: marketingskills Lifecycle Kernel
...
export class marketingskillsLifecycleContext {
  public readonly id: string;
  public readonly parent: marketingskillsLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: marketingskillsLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: marketingskillsLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[marketingskillsLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: marketingskillsLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[marketingskillsLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): marketingskillsLifecycleContext {
    return new marketingskillsLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[marketingskillsLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: marketingskills ReAct Loop Engine
...
export class marketingskillsAgentLoopEngine {
  constructor(
    private ctx: marketingskillsLifecycleContext,
...
## Engine 3: marketingskills Unified Model Stream Adapter
...
export class marketingskillsModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[marketingskillsModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: marketingskills Tool Sandbox & Virtual File System Engine
...
export class marketingskillsVirtualFileSystem {
...
export class marketingskillsToolSandbox {
  private vfs: marketingskillsVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new marketingskillsVirtualFileSystem(initialFiles);
  }
...
## Engine 5: marketingskills Non-Linear Session Tree & Token Budget Engine
...
export class marketingskillsSessionTreeEngine {
