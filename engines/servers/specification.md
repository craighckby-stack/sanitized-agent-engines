...
> **Source Origin**: [modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers)
...
## Engine 1: servers Lifecycle Kernel
...
export class serversLifecycleContext {
  public readonly id: string;
  public readonly parent: serversLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: serversLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: serversLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[serversLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: serversLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[serversLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): serversLifecycleContext {
    return new serversLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[serversLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: servers ReAct Loop Engine
...
export class serversAgentLoopEngine {
  constructor(
    private ctx: serversLifecycleContext,
...
## Engine 3: servers Unified Model Stream Adapter
...
export class serversModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[serversModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: servers Tool Sandbox & Virtual File System Engine
...
export class serversVirtualFileSystem {
...
export class serversToolSandbox {
  private vfs: serversVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new serversVirtualFileSystem(initialFiles);
  }
...
## Engine 5: servers Non-Linear Session Tree & Token Budget Engine
...
export class serversSessionTreeEngine {
