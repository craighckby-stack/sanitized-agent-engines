...
> **Source Origin**: [calesthio/OpenMontage](https://github.com/calesthio/OpenMontage)
...
## Engine 1: OpenMontage Lifecycle Kernel
...
export class OpenMontageLifecycleContext {
  public readonly id: string;
  public readonly parent: OpenMontageLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: OpenMontageLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: OpenMontageLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[OpenMontageLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: OpenMontageLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[OpenMontageLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): OpenMontageLifecycleContext {
    return new OpenMontageLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[OpenMontageLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: OpenMontage ReAct Loop Engine
...
export class OpenMontageAgentLoopEngine {
  constructor(
    private ctx: OpenMontageLifecycleContext,
...
## Engine 3: OpenMontage Unified Model Stream Adapter
...
export class OpenMontageModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[OpenMontageModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: OpenMontage Tool Sandbox & Virtual File System Engine
...
export class OpenMontageVirtualFileSystem {
...
export class OpenMontageToolSandbox {
  private vfs: OpenMontageVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new OpenMontageVirtualFileSystem(initialFiles);
  }
...
## Engine 5: OpenMontage Non-Linear Session Tree & Token Budget Engine
...
export class OpenMontageSessionTreeEngine {
