...
> **Source Origin**: [Significant-Gravitas/AutoGPT](https://github.com/Significant-Gravitas/AutoGPT)
...
## Engine 1: AutoGPT Lifecycle Kernel
...
export class AutoGPTLifecycleContext {
  public readonly id: string;
  public readonly parent: AutoGPTLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: AutoGPTLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: AutoGPTLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[AutoGPTLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: AutoGPTLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[AutoGPTLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): AutoGPTLifecycleContext {
    return new AutoGPTLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[AutoGPTLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: AutoGPT ReAct Loop Engine
...
export class AutoGPTAgentLoopEngine {
  constructor(
    private ctx: AutoGPTLifecycleContext,
...
## Engine 3: AutoGPT Unified Model Stream Adapter
...
export class AutoGPTModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[AutoGPTModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: AutoGPT Tool Sandbox & Virtual File System Engine
...
export class AutoGPTVirtualFileSystem {
...
export class AutoGPTToolSandbox {
  private vfs: AutoGPTVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new AutoGPTVirtualFileSystem(initialFiles);
  }
...
## Engine 5: AutoGPT Non-Linear Session Tree & Token Budget Engine
