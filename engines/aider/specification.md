...
> **Source Origin**: [Aider-AI/aider](https://github.com/Aider-AI/aider)
...
## Engine 1: aider Lifecycle Kernel
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[aiderLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[aiderLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: aider ReAct Loop Engine
...
## Engine 3: aider Unified Model Stream Adapter
...
export class aiderModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[aiderModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: aider Tool Sandbox & Virtual File System Engine
...
export class aiderVirtualFileSystem {
...
export class aiderToolSandbox {
  private vfs: aiderVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new aiderVirtualFileSystem(initialFiles);
  }
...
## Engine 5: aider Non-Linear Session Tree & Token Budget Engine
...
export class aiderSessionTreeEngine {
