...
## Engine 1: phidata Lifecycle Kernel
...
export class phidataLifecycleContext {
  public readonly id: string;
  public readonly parent: phidataLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: phidataLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: phidataLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[phidataLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: phidataLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[phidataLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): phidataLifecycleContext {
    return new phidataLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[phidataLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: phidata ReAct Loop Engine
...
export class phidataAgentLoopEngine {
  constructor(
    private ctx: phidataLifecycleContext,
...
## Engine 3: phidata Unified Model Stream Adapter
...
export class phidataModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[phidataModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: phidata Tool Sandbox & Virtual File System Engine
...
export class phidataVirtualFileSystem {
...
export class phidataToolSandbox {
  private vfs: phidataVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new phidataVirtualFileSystem(initialFiles);
  }
...
## Engine 5: phidata Non-Linear Session Tree & Token Budget Engine
...
export class phidataSessionTreeEngine {
