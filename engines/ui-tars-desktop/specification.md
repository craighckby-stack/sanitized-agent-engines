...
> **Source Origin**: [bytedance/UI-TARS-desktop](https://github.com/bytedance/UI-TARS-desktop)
...
## Engine 1: UI-TARS-desktop Lifecycle Kernel
...
export class UI-TARS-desktopLifecycleContext {
  public readonly id: string;
  public readonly parent: UI-TARS-desktopLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: UI-TARS-desktopLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: UI-TARS-desktopLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[UI-TARS-desktopLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: UI-TARS-desktopLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[UI-TARS-desktopLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): UI-TARS-desktopLifecycleContext {
    return new UI-TARS-desktopLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[UI-TARS-desktopLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: UI-TARS-desktop ReAct Loop Engine
...
export class UI-TARS-desktopAgentLoopEngine {
  constructor(
    private ctx: UI-TARS-desktopLifecycleContext,
...
## Engine 3: UI-TARS-desktop Unified Model Stream Adapter
...
export class UI-TARS-desktopModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[UI-TARS-desktopModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: UI-TARS-desktop Tool Sandbox & Virtual File System Engine
...
export class UI-TARS-desktopVirtualFileSystem {
...
export class UI-TARS-desktopToolSandbox {
  private vfs: UI-TARS-desktopVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new UI-TARS-desktopVirtualFileSystem(initialFiles);
  }
...
## Engine 5: UI-TARS-desktop Non-Linear Session Tree & Token Budget Engine
...
export class UI-TARS-desktopSessionTreeEngine {
