...
> **Source Origin**: [CherryHQ/cherry-studio](https://github.com/CherryHQ/cherry-studio)
...
## Engine 1: cherry-studio Lifecycle Kernel
...
export class cherry-studioLifecycleContext {
  public readonly id: string;
  public readonly parent: cherry-studioLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: cherry-studioLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: cherry-studioLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[cherry-studioLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: cherry-studioLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[cherry-studioLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): cherry-studioLifecycleContext {
    return new cherry-studioLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[cherry-studioLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: cherry-studio ReAct Loop Engine
...
export class cherry-studioAgentLoopEngine {
  constructor(
    private ctx: cherry-studioLifecycleContext,
...
## Engine 3: cherry-studio Unified Model Stream Adapter
...
export class cherry-studioModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[cherry-studioModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: cherry-studio Tool Sandbox & Virtual File System Engine
...
export class cherry-studioVirtualFileSystem {
...
export class cherry-studioToolSandbox {
  private vfs: cherry-studioVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new cherry-studioVirtualFileSystem(initialFiles);
  }
...
## Engine 5: cherry-studio Non-Linear Session Tree & Token Budget Engine
...
export class cherry-studioSessionTreeEngine {
