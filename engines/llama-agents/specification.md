...
> **Source Origin**: [run-llama/llama-agents](https://github.com/run-llama/llama-agents)
...
## Engine 1: llama-agents Lifecycle Kernel
...
export class llama-agentsLifecycleContext {
  public readonly id: string;
  public readonly parent: llama-agentsLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: llama-agentsLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: llama-agentsLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[llama-agentsLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: llama-agentsLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[llama-agentsLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): llama-agentsLifecycleContext {
    return new llama-agentsLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[llama-agentsLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: llama-agents ReAct Loop Engine
...
export class llama-agentsAgentLoopEngine {
  constructor(
    private ctx: llama-agentsLifecycleContext,
...
## Engine 3: llama-agents Unified Model Stream Adapter
...
export class llama-agentsModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[llama-agentsModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: llama-agents Tool Sandbox & Virtual File System Engine
...
export class llama-agentsVirtualFileSystem {
...
export class llama-agentsToolSandbox {
  private vfs: llama-agentsVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new llama-agentsVirtualFileSystem(initialFiles);
  }
...
## Engine 5: llama-agents Non-Linear Session Tree & Token Budget Engine
...
export class llama-agentsSessionTreeEngine {
