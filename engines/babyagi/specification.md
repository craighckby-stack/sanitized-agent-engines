...
## Engine 1: babyagi Lifecycle Kernel
...
export class babyagiLifecycleContext {
  public readonly id: string;
  public readonly parent: babyagiLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: babyagiLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: babyagiLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[babyagiLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: babyagiLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[babyagiLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): babyagiLifecycleContext {
    return new babyagiLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[babyagiLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: babyagi ReAct Loop Engine
...
export class babyagiAgentLoopEngine {
  constructor(
    private ctx: babyagiLifecycleContext,
...
## Engine 3: babyagi Unified Model Stream Adapter
...
export class babyagiModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[babyagiModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: babyagi Tool Sandbox & Virtual File System Engine
...
export class babyagiVirtualFileSystem {
...
export class babyagiToolSandbox {
  private vfs: babyagiVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new babyagiVirtualFileSystem(initialFiles);
  }
...
## Engine 5: babyagi Non-Linear Session Tree & Token Budget Engine
...
export class babyagiSessionTreeEngine {
