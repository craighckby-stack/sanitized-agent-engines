...
## Engine 1: claude-seo Lifecycle Kernel
...
export class claude-seoLifecycleContext {
  public readonly id: string;
  public readonly parent: claude-seoLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: claude-seoLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: claude-seoLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[claude-seoLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: claude-seoLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[claude-seoLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): claude-seoLifecycleContext {
    return new claude-seoLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[claude-seoLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: claude-seo ReAct Loop Engine
...
export class claude-seoAgentLoopEngine {
  constructor(
    private ctx: claude-seoLifecycleContext,
...
## Engine 3: claude-seo Unified Model Stream Adapter
...
export class claude-seoModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[claude-seoModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: claude-seo Tool Sandbox & Virtual File System Engine
...
## Engine 5: claude-seo Non-Linear Session Tree & Token Budget Engine
...
export class claude-seoSessionTreeEngine {
