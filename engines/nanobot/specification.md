...
> **Source Origin**: [HKUDS/nanobot](https://github.com/HKUDS/nanobot)
...
## Engine 1: nanobot Lifecycle Kernel
...
export class nanobotLifecycleContext {
  public readonly id: string;
  public readonly parent: nanobotLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: nanobotLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: nanobotLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[nanobotLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: nanobotLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[nanobotLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): nanobotLifecycleContext {
    return new nanobotLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[nanobotLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: nanobot ReAct Loop Engine
...
export class nanobotAgentLoopEngine {
  constructor(
    private ctx: nanobotLifecycleContext,
...
## Engine 3: nanobot Unified Model Stream Adapter
...
export class nanobotModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[nanobotModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: nanobot Tool Sandbox & Virtual File System Engine
...
export class nanobotVirtualFileSystem {
...
export class nanobotToolSandbox {
  private vfs: nanobotVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new nanobotVirtualFileSystem(initialFiles);
  }
...
## Engine 5: nanobot Non-Linear Session Tree & Token Budget Engine
...
export class nanobotSessionTreeEngine {
