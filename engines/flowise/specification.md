...
> **Source Origin**: [FlowiseAI/Flowise](https://github.com/FlowiseAI/Flowise)
...
## Engine 1: Flowise Lifecycle Kernel
...
export class FlowiseLifecycleContext {
  public readonly id: string;
  public readonly parent: FlowiseLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: FlowiseLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: FlowiseLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[FlowiseLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: FlowiseLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[FlowiseLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): FlowiseLifecycleContext {
    return new FlowiseLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[FlowiseLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: Flowise ReAct Loop Engine
...
export class FlowiseAgentLoopEngine {
  constructor(
    private ctx: FlowiseLifecycleContext,
...
## Engine 3: Flowise Unified Model Stream Adapter
...
export class FlowiseModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[FlowiseModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: Flowise Tool Sandbox & Virtual File System Engine
...
export class FlowiseVirtualFileSystem {
...
export class FlowiseToolSandbox {
  private vfs: FlowiseVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new FlowiseVirtualFileSystem(initialFiles);
  }
...
## Engine 5: Flowise Non-Linear Session Tree & Token Budget Engine
...
export class FlowiseSessionTreeEngine {
