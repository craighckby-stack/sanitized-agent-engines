...
> **Source Origin**: [microsoft/qlib](https://github.com/microsoft/qlib)
...
## Engine 1: qlib Lifecycle Kernel
...
export class qlibLifecycleContext {
  public readonly id: string;
  public readonly parent: qlibLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: qlibLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: qlibLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[qlibLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: qlibLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[qlibLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): qlibLifecycleContext {
    return new qlibLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[qlibLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: qlib ReAct Loop Engine
...
export class qlibAgentLoopEngine {
  constructor(
    private ctx: qlibLifecycleContext,
...
## Engine 3: qlib Unified Model Stream Adapter
...
export class qlibModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[qlibModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: qlib Tool Sandbox & Virtual File System Engine
...
export class qlibVirtualFileSystem {
...
export class qlibToolSandbox {
  private vfs: qlibVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new qlibVirtualFileSystem(initialFiles);
  }
...
## Engine 5: qlib Non-Linear Session Tree & Token Budget Engine
...
export class qlibSessionTreeEngine {
