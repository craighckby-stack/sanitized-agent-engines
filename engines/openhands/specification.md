...
## Engine 1: OpenHands Lifecycle Kernel
...
export class OpenHandsLifecycleContext {
  public readonly id: string;
  public readonly parent: OpenHandsLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: OpenHandsLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: OpenHandsLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[OpenHandsLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: OpenHandsLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[OpenHandsLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): OpenHandsLifecycleContext {
    return new OpenHandsLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[OpenHandsLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: OpenHands ReAct Loop Engine
...
export class OpenHandsAgentLoopEngine {
  constructor(
    private ctx: OpenHandsLifecycleContext,
...
## Engine 3: OpenHands Unified Model Stream Adapter
...
export class OpenHandsModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[OpenHandsModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: OpenHands Tool Sandbox & Virtual File System Engine
...
export class OpenHandsVirtualFileSystem {
...
export class OpenHandsToolSandbox {
  private vfs: OpenHandsVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new OpenHandsVirtualFileSystem(initialFiles);
  }
...
## Engine 5: OpenHands Non-Linear Session Tree & Token Budget Engine
...
export class OpenHandsSessionTreeEngine {
