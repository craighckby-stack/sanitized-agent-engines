...
> **Source Origin**: [Hmbown/Codewhale](https://github.com/Hmbown/Codewhale)
...
## Engine 1: Codewhale Lifecycle Kernel
...
export class CodewhaleLifecycleContext {
  public readonly id: string;
  public readonly parent: CodewhaleLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: CodewhaleLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: CodewhaleLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[CodewhaleLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: CodewhaleLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[CodewhaleLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): CodewhaleLifecycleContext {
    return new CodewhaleLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[CodewhaleLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: Codewhale ReAct Loop Engine
...
export class CodewhaleAgentLoopEngine {
  constructor(
    private ctx: CodewhaleLifecycleContext,
...
## Engine 3: Codewhale Unified Model Stream Adapter
...
export class CodewhaleModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[CodewhaleModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: Codewhale Tool Sandbox & Virtual File System Engine
...
export class CodewhaleVirtualFileSystem {
...
export class CodewhaleToolSandbox {
  private vfs: CodewhaleVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new CodewhaleVirtualFileSystem(initialFiles);
  }
...
## Engine 5: Codewhale Non-Linear Session Tree & Token Budget Engine
...
export class CodewhaleSessionTreeEngine {
