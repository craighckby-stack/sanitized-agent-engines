...
## Engine 1: anything-llm Lifecycle Kernel
...
export class anything-llmLifecycleContext {
  public readonly id: string;
  public readonly parent: anything-llmLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: anything-llmLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: anything-llmLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[anything-llmLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: anything-llmLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[anything-llmLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): anything-llmLifecycleContext {
    return new anything-llmLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[anything-llmLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: anything-llm ReAct Loop Engine
...
export class anything-llmAgentLoopEngine {
  constructor(
    private ctx: anything-llmLifecycleContext,
...
## Engine 3: anything-llm Unified Model Stream Adapter
...
export class anything-llmModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[anything-llmModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: anything-llm Tool Sandbox & Virtual File System Engine
...
export class anything-llmVirtualFileSystem {
...
export class anything-llmToolSandbox {
  private vfs: anything-llmVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new anything-llmVirtualFileSystem(initialFiles);
  }
...
## Engine 5: anything-llm Non-Linear Session Tree & Token Budget Engine
...
export class anything-llmSessionTreeEngine {
