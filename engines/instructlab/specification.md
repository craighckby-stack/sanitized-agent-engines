...
> **Source Origin**: [instructlab/instructlab](https://github.com/instructlab/instructlab)
...
## Engine 1: instructlab Lifecycle Kernel
...
export class instructlabLifecycleContext {
  public readonly id: string;
  public readonly parent: instructlabLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: instructlabLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: instructlabLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[instructlabLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: instructlabLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[instructlabLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): instructlabLifecycleContext {
    return new instructlabLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[instructlabLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: instructlab ReAct Loop Engine
...
export class instructlabAgentLoopEngine {
  constructor(
    private ctx: instructlabLifecycleContext,
...
## Engine 3: instructlab Unified Model Stream Adapter
...
export class instructlabModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[instructlabModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: instructlab Tool Sandbox & Virtual File System Engine
...
export class instructlabVirtualFileSystem {
...
export class instructlabToolSandbox {
  private vfs: instructlabVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new instructlabVirtualFileSystem(initialFiles);
  }
...
## Engine 5: instructlab Non-Linear Session Tree & Token Budget Engine
...
export class instructlabSessionTreeEngine {
