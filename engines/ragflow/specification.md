...
## Engine 1: ragflow Lifecycle Kernel
...
export class ragflowLifecycleContext {
  public readonly id: string;
  public readonly parent: ragflowLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: ragflowLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: ragflowLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[ragflowLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: ragflowLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[ragflowLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): ragflowLifecycleContext {
    return new ragflowLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[ragflowLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: ragflow ReAct Loop Engine
...
export class ragflowAgentLoopEngine {
  constructor(
    private ctx: ragflowLifecycleContext,
...
## Engine 3: ragflow Unified Model Stream Adapter
...
export class ragflowModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[ragflowModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: ragflow Tool Sandbox & Virtual File System Engine
...
export class ragflowVirtualFileSystem {
...
export class ragflowToolSandbox {
  private vfs: ragflowVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new ragflowVirtualFileSystem(initialFiles);
  }
...
## Engine 5: ragflow Non-Linear Session Tree & Token Budget Engine
...
export class ragflowSessionTreeEngine {
