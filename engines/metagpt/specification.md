...
## Engine 1: MetaGPT Lifecycle Kernel
...
export class MetaGPTLifecycleContext {
  public readonly id: string;
  public readonly parent: MetaGPTLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: MetaGPTLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: MetaGPTLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[MetaGPTLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: MetaGPTLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[MetaGPTLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): MetaGPTLifecycleContext {
    return new MetaGPTLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[MetaGPTLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: MetaGPT ReAct Loop Engine
...
export class MetaGPTAgentLoopEngine {
  constructor(
    private ctx: MetaGPTLifecycleContext,
...
## Engine 3: MetaGPT Unified Model Stream Adapter
...
## Engine 4: MetaGPT Tool Sandbox & Virtual File System Engine
...
export class MetaGPTVirtualFileSystem {
...
export class MetaGPTToolSandbox {
  private vfs: MetaGPTVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new MetaGPTVirtualFileSystem(initialFiles);
  }
...
## Engine 5: MetaGPT Non-Linear Session Tree & Token Budget Engine
...
export class MetaGPTSessionTreeEngine {
