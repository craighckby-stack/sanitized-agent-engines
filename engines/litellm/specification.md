...
> **Source Origin**: [BerriAI/litellm](https://github.com/BerriAI/litellm)
...
## Engine 1: litellm Lifecycle Kernel
...
export class litellmLifecycleContext {
  public readonly id: string;
  public readonly parent: litellmLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: litellmLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: litellmLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[litellmLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: litellmLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[litellmLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): litellmLifecycleContext {
    return new litellmLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[litellmLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: litellm ReAct Loop Engine
...
export class litellmAgentLoopEngine {
  constructor(
    private ctx: litellmLifecycleContext,
...
## Engine 3: litellm Unified Model Stream Adapter
...
export class litellmModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[litellmModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: litellm Tool Sandbox & Virtual File System Engine
...
export class litellmVirtualFileSystem {
...
export class litellmToolSandbox {
  private vfs: litellmVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new litellmVirtualFileSystem(initialFiles);
  }
...
## Engine 5: litellm Non-Linear Session Tree & Token Budget Engine
...
export class litellmSessionTreeEngine {
