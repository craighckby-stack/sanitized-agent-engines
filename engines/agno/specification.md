...
## Engine 1: agno Lifecycle Kernel
...
export class agnoLifecycleContext {
  public readonly id: string;
  public readonly parent: agnoLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: agnoLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: agnoLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[agnoLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: agnoLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[agnoLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): agnoLifecycleContext {
    return new agnoLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[agnoLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: agno ReAct Loop Engine
...
export class agnoAgentLoopEngine {
  constructor(
    private ctx: agnoLifecycleContext,
...
## Engine 3: agno Unified Model Stream Adapter
...
export class agnoModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[agnoModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: agno Tool Sandbox & Virtual File System Engine
...
export class agnoVirtualFileSystem {
...
export class agnoToolSandbox {
  private vfs: agnoVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new agnoVirtualFileSystem(initialFiles);
  }
...
## Engine 5: agno Non-Linear Session Tree & Token Budget Engine
...
export class agnoSessionTreeEngine {
