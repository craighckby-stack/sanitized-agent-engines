...
> **Source Origin**: [volcengine/OpenViking](https://github.com/volcengine/OpenViking)
...
## Engine 1: OpenViking Lifecycle Kernel
...
export class OpenVikingLifecycleContext {
  public readonly id: string;
  public readonly parent: OpenVikingLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: OpenVikingLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: OpenVikingLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[OpenVikingLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: OpenVikingLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[OpenVikingLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): OpenVikingLifecycleContext {
    return new OpenVikingLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[OpenVikingLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: OpenViking ReAct Loop Engine
...
export class OpenVikingAgentLoopEngine {
  constructor(
    private ctx: OpenVikingLifecycleContext,
...
## Engine 3: OpenViking Unified Model Stream Adapter
...
export class OpenVikingModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[OpenVikingModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: OpenViking Tool Sandbox & Virtual File System Engine
...
export class OpenVikingVirtualFileSystem {
...
export class OpenVikingToolSandbox {
  private vfs: OpenVikingVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new OpenVikingVirtualFileSystem(initialFiles);
  }
...
## Engine 5: OpenViking Non-Linear Session Tree & Token Budget Engine
...
export class OpenVikingSessionTreeEngine {
