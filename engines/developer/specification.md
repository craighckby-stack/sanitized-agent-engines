...
## Engine 1: developer Lifecycle Kernel
...
export class developerLifecycleContext {
  public readonly id: string;
  public readonly parent: developerLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: developerLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: developerLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[developerLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: developerLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[developerLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): developerLifecycleContext {
    return new developerLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[developerLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: developer ReAct Loop Engine
...
export class developerAgentLoopEngine {
  constructor(
    private ctx: developerLifecycleContext,
...
## Engine 3: developer Unified Model Stream Adapter
...
export class developerModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[developerModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: developer Tool Sandbox & Virtual File System Engine
...
export class developerVirtualFileSystem {
...
export class developerToolSandbox {
  private vfs: developerVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new developerVirtualFileSystem(initialFiles);
  }
...
## Engine 5: developer Non-Linear Session Tree & Token Budget Engine
...
export class developerSessionTreeEngine {
