...
> **Source Origin**: [mvanhorn/last30days-skill](https://github.com/mvanhorn/last30days-skill)
...
## Engine 1: last30days-skill Lifecycle Kernel
...
export class last30days-skillLifecycleContext {
  public readonly id: string;
  public readonly parent: last30days-skillLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: last30days-skillLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: last30days-skillLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[last30days-skillLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: last30days-skillLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[last30days-skillLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): last30days-skillLifecycleContext {
    return new last30days-skillLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[last30days-skillLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: last30days-skill ReAct Loop Engine
...
export class last30days-skillAgentLoopEngine {
  constructor(
    private ctx: last30days-skillLifecycleContext,
...
## Engine 3: last30days-skill Unified Model Stream Adapter
...
export class last30days-skillModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[last30days-skillModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: last30days-skill Tool Sandbox & Virtual File System Engine
...
export class last30days-skillVirtualFileSystem {
...
export class last30days-skillToolSandbox {
  private vfs: last30days-skillVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new last30days-skillVirtualFileSystem(initialFiles);
  }
...
## Engine 5: last30days-skill Non-Linear Session Tree & Token Budget Engine
...
export class last30days-skillSessionTreeEngine {
