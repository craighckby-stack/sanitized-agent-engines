...
> **Source Origin**: [stitionai/devika](https://github.com/stitionai/devika)
...
## Engine 1: devika Lifecycle Kernel
...
export class devikaLifecycleContext {
  public readonly id: string;
  public readonly parent: devikaLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: devikaLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: devikaLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[devikaLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: devikaLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[devikaLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): devikaLifecycleContext {
    return new devikaLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[devikaLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: devika ReAct Loop Engine
...
export class devikaAgentLoopEngine {
  constructor(
    private ctx: devikaLifecycleContext,
...
## Engine 3: devika Unified Model Stream Adapter
...
export class devikaModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[devikaModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: devika Tool Sandbox & Virtual File System Engine
...
export class devikaVirtualFileSystem {
...
export class devikaToolSandbox {
  private vfs: devikaVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new devikaVirtualFileSystem(initialFiles);
  }
...
## Engine 5: devika Non-Linear Session Tree & Token Budget Engine
...
export class devikaSessionTreeEngine {
