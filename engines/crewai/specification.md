...
> **Source Origin**: [crewAIInc/crewAI](https://github.com/crewAIInc/crewAI)
...
## Engine 1: crewAI Lifecycle Kernel
...
export class crewAILifecycleContext {
  public readonly id: string;
  public readonly parent: crewAILifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: crewAILifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: crewAILifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[crewAILifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: crewAILifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[crewAILifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): crewAILifecycleContext {
    return new crewAILifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[crewAILifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: crewAI ReAct Loop Engine
...
export class crewAIAgentLoopEngine {
  constructor(
    private ctx: crewAILifecycleContext,
...
## Engine 3: crewAI Unified Model Stream Adapter
...
export class crewAIModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[crewAIModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: crewAI Tool Sandbox & Virtual File System Engine
...
export class crewAIVirtualFileSystem {
...
export class crewAIToolSandbox {
  private vfs: crewAIVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new crewAIVirtualFileSystem(initialFiles);
  }
...
## Engine 5: crewAI Non-Linear Session Tree & Token Budget Engine
...
export class crewAISessionTreeEngine {
