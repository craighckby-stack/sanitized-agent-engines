...
> **Source Origin**: [hugohe3/ppt-master](https://github.com/hugohe3/ppt-master)
...
## Engine 1: ppt-master Lifecycle Kernel
...
export class ppt-masterLifecycleContext {
  public readonly id: string;
  public readonly parent: ppt-masterLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: ppt-masterLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: ppt-masterLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[ppt-masterLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: ppt-masterLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[ppt-masterLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): ppt-masterLifecycleContext {
    return new ppt-masterLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[ppt-masterLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: ppt-master ReAct Loop Engine
...
export class ppt-masterAgentLoopEngine {
  constructor(
    private ctx: ppt-masterLifecycleContext,
...
## Engine 3: ppt-master Unified Model Stream Adapter
...
export class ppt-masterModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[ppt-masterModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: ppt-master Tool Sandbox & Virtual File System Engine
...
export class ppt-masterVirtualFileSystem {
...
export class ppt-masterToolSandbox {
  private vfs: ppt-masterVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new ppt-masterVirtualFileSystem(initialFiles);
  }
...
## Engine 5: ppt-master Non-Linear Session Tree & Token Budget Engine
...
export class ppt-masterSessionTreeEngine {
