...
> **Source Origin**: [siyuan-note/siyuan](https://github.com/siyuan-note/siyuan)
...
## Engine 1: siyuan Lifecycle Kernel
...
export class siyuanLifecycleContext {
  ...
  public readonly parent: siyuanLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: siyuanLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: siyuanLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[siyuanLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: siyuanLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[siyuanLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): siyuanLifecycleContext {
    return new siyuanLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[siyuanLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: siyuan ReAct Loop Engine
...
## Engine 3: siyuan Unified Model Stream Adapter
...
export class siyuanModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[siyuanModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: siyuan Tool Sandbox & Virtual File System Engine
...
export class siyuanVirtualFileSystem {
...
export class siyuanToolSandbox {
  private vfs: siyuanVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new siyuanVirtualFileSystem(initialFiles);
  }
...
## Engine 5: siyuan Non-Linear Session Tree & Token Budget Engine
...
export class siyuanSessionTreeEngine {
