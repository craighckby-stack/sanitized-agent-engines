...
> **Source Origin**: [AstrBotDevs/AstrBot](https://github.com/AstrBotDevs/AstrBot)
...
## Engine 1: AstrBot Lifecycle Kernel
...
export class AstrBotLifecycleContext {
  public readonly id: string;
  public readonly parent: AstrBotLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: AstrBotLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: AstrBotLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[AstrBotLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: AstrBotLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[AstrBotLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): AstrBotLifecycleContext {
    return new AstrBotLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[AstrBotLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: AstrBot ReAct Loop Engine
...
export class AstrBotAgentLoopEngine {
  constructor(
    private ctx: AstrBotLifecycleContext,
...
## Engine 3: AstrBot Unified Model Stream Adapter
...
export class AstrBotModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[AstrBotModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: AstrBot Tool Sandbox & Virtual File System Engine
...
export class AstrBotVirtualFileSystem {
...
export class AstrBotToolSandbox {
  private vfs: AstrBotVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new AstrBotVirtualFileSystem(initialFiles);
  }
...
## Engine 5: AstrBot Non-Linear Session Tree & Token Budget Engine
...
export class AstrBotSessionTreeEngine {
