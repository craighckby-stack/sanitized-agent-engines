...
> **Source Origin**: [blader/humanizer](https://github.com/blader/humanizer)
...
## Engine 1: humanizer Lifecycle Kernel
...
export class humanizerLifecycleContext {
  public readonly id: string;
  public readonly parent: humanizerLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: humanizerLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: humanizerLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[humanizerLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: humanizerLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[humanizerLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): humanizerLifecycleContext {
    return new humanizerLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[humanizerLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: humanizer ReAct Loop Engine
...
export class humanizerAgentLoopEngine {
  constructor(
    private ctx: humanizerLifecycleContext,
...
## Engine 3: humanizer Unified Model Stream Adapter
...
export class humanizerModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[humanizerModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: humanizer Tool Sandbox & Virtual File System Engine
...
export class humanizerVirtualFileSystem {
...
export class humanizerToolSandbox {
  private vfs: humanizerVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new humanizerVirtualFileSystem(initialFiles);
  }
...
## Engine 5: humanizer Non-Linear Session Tree & Token Budget Engine
...
export class humanizerSessionTreeEngine {
