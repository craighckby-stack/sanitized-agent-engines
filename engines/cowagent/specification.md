...
> **Source Origin**: [zhayujie/CowAgent](https://github.com/zhayujie/CowAgent)
...
## Engine 1: CowAgent Lifecycle Kernel
...
export class CowAgentLifecycleContext {
  public readonly id: string;
  public readonly parent: CowAgentLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: CowAgentLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: CowAgentLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[CowAgentLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: CowAgentLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[CowAgentLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): CowAgentLifecycleContext {
    return new CowAgentLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[CowAgentLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: CowAgent ReAct Loop Engine
...
export class CowAgentAgentLoopEngine {
  constructor(
    private ctx: CowAgentLifecycleContext,
...
## Engine 3: CowAgent Unified Model Stream Adapter
...
export class CowAgentModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[CowAgentModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: CowAgent Tool Sandbox & Virtual File System Engine
...
export class CowAgentVirtualFileSystem {
...
export class CowAgentToolSandbox {
  private vfs: CowAgentVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new CowAgentVirtualFileSystem(initialFiles);
  }
...
## Engine 5: CowAgent Non-Linear Session Tree & Token Budget Engine
...
export class CowAgentSessionTreeEngine {
