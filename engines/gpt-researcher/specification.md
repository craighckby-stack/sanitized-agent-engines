...
> **Source Origin**: [assafelovic/gpt-researcher](https://github.com/assafelovic/gpt-researcher)
...
## Engine 1: gpt-researcher Lifecycle Kernel
...
export class gpt-researcherLifecycleContext {
  public readonly id: string;
  public readonly parent: gpt-researcherLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: gpt-researcherLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: gpt-researcherLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[gpt-researcherLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: gpt-researcherLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[gpt-researcherLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): gpt-researcherLifecycleContext {
    return new gpt-researcherLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[gpt-researcherLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: gpt-researcher ReAct Loop Engine
...
export class gpt-researcherAgentLoopEngine {
  constructor(
    private ctx: gpt-researcherLifecycleContext,
...
## Engine 3: gpt-researcher Unified Model Stream Adapter
...
export class gpt-researcherModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[gpt-researcherModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: gpt-researcher Tool Sandbox & Virtual File System Engine
...
export class gpt-researcherVirtualFileSystem {
...
export class gpt-researcherToolSandbox {
  private vfs: gpt-researcherVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new gpt-researcherVirtualFileSystem(initialFiles);
  }
...
## Engine 5: gpt-researcher Non-Linear Session Tree & Token Budget Engine
...
export class gpt-researcherSessionTreeEngine {
