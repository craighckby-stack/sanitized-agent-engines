# scientific-agent-skills Specification
...
> **Source Origin**: [K-Dense-AI/scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills)
...
## Engine 1: scientific-agent-skills Lifecycle Kernel
...
export class scientific-agent-skillsLifecycleContext {
  public readonly id: string;
  public readonly parent: scientific-agent-skillsLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: scientific-agent-skillsLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: scientific-agent-skillsLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[scientific-agent-skillsLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: scientific-agent-skillsLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[scientific-agent-skillsLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): scientific-agent-skillsLifecycleContext {
    return new scientific-agent-skillsLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[scientific-agent-skillsLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: scientific-agent-skills ReAct Loop Engine
...
export class scientific-agent-skillsAgentLoopEngine {
  constructor(
    private ctx: scientific-agent-skillsLifecycleContext,
...
## Engine 3: scientific-agent-skills Unified Model Stream Adapter
...
export class scientific-agent-skillsModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[scientific-agent-skillsModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: scientific-agent-skills Tool Sandbox & Virtual File System Engine
...
export class scientific-agent-skillsVirtualFileSystem {
...
export class scientific-agent-skillsToolSandbox {
  private vfs: scientific-agent-skillsVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new scientific-agent-skillsVirtualFileSystem(initialFiles);
  }
...
## Engine 5: scientific-agent-skills Non-Linear Session Tree & Token Budget Engine
...
export class scientific-agent-skillsSessionTreeEngine {
