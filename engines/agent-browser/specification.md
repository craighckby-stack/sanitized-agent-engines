...
> **Source Origin**: [vercel-labs/agent-browser](https://github.com/vercel-labs/agent-browser)
...
## Engine 1: agent-browser Lifecycle Kernel
...
export class agent-browserLifecycleContext {
  public readonly id: string;
  public readonly parent: agent-browserLifecycleContext | null;
  public readonly scope: 'global' | 'session' | 'step';
  private services = new Map<string, unknown>();
  private hooks = new Map<string, Set<(payload: any, ctx: agent-browserLifecycleContext) => void | Promise<void>>>();
  private disposables = new Set<Disposable>();

  constructor(scope: 'global' | 'session' | 'step' = 'global', parent: agent-browserLifecycleContext | null = null) {
  ...
  public inject<T>(id: string): T {
    ...
    throw new Error(`[agent-browserLifecycleContext] Service '${id}' not registered in context hierarchy.`);
  }
  ...
  public on<T>(event: LifecycleHookName, handler: (payload: T, ctx: agent-browserLifecycleContext) => void | Promise<void>): Disposable {
    ...
    if (handlers && handlers.size > 0) {
      for (const h of Array.from(handlers)) {
        try {
          ...
          console.error(`[agent-browserLifecycleContext] Error in hook '${event}':`, err);
        }
      }
    }
    if (this.parent) {
  ...
  public extend(scope: 'session' | 'step'): agent-browserLifecycleContext {
    return new agent-browserLifecycleContext(scope, this);
  }
  ...
  public async dispose(): Promise<void> {
    ...
    for (const d of disposables) {
      try {
        ...
        console.warn(`[agent-browserLifecycleContext] Dispose error:`, err);
      }
    }
    this.hooks.clear();
    this.services.clear();
...
## Engine 2: agent-browser ReAct Loop Engine
...
export class agent-browserAgentLoopEngine {
  constructor(
    private ctx: agent-browserLifecycleContext,
...
## Engine 3: agent-browser Unified Model Stream Adapter
...
export class agent-browserModelAdapter {
  ...
  public async *generateStream(messages: any[], tools: any[]): AsyncIterable<StreamDelta> {
    try {
      ...
      yield {
        type: 'thought_chunk',
        deltaThought: `[agent-browserModelAdapter] Reasoning offline: Analyzing execution invariant for ${messages.length} messages.\n`,
      };
      yield {
        type: 'text_chunk',
...
## Engine 4: agent-browser Tool Sandbox & Virtual File System Engine
...
export class agent-browserVirtualFileSystem {
...
export class agent-browserToolSandbox {
  private vfs: agent-browserVirtualFileSystem;

  constructor(initialFiles: Record<string, string> = {}) {
    this.vfs = new agent-browserVirtualFileSystem(initialFiles);
  }
...
## Engine 5: agent-browser Non-Linear Session Tree & Token Budget Engine
...
export class agent-browserSessionTreeEngine {
